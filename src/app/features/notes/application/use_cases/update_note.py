from src.app.features.notes.application.dtos.note_dto import NoteResponse, NoteUpdateRequest
...
class UpdateNoteUseCase:
    """Partially update a note's content or type, validating ownership."""

    def __init__(self, note_repository: NoteRepository, project_repository: ProjectRepository):
        self._note_repository = note_repository
        self._project_repository = project_repository

    async def execute(self, note_id: str, owner_id: str, payload: NoteUpdateRequest) -> NoteResponse:
        ...
        if payload.content is not None:
            note.update_content(payload.content.strip())

        updated = await self._note_repository.update(note)
